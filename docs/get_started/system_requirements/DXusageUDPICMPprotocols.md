# Actian DX Usage of UDP and ICMP Protocols

On a default Actian DX installation, UDP transport chains are disabled. Some versions of Actian Sametime used UDP. However, on a running system, some UDP ports may appear as listening:
...
Default Behavior: Actian DX does not require UDP or ICMP for normal operation. TCP is used for all core communications.

- UDP Ports Observed: Ports `5000` and `5001` are used by WAS for internal node discovery. They are not required for Actian DX services outside the cluster.  
- WebSphere_Portal Process: The `1506286/java` process may open UDP ports for WAS internal operations.  
- Unknown Ports: Port `10045` is configured by WAS but does not affect Actian DX functionality; review in context of internal security policies.  
- Security Implication: Disabling these UDP ports externally does not affect Actian DX normal operations. These ports are relevant only for intra-cluster node discovery within the WAS environment.  
- ICMP: No Actian DX component requires ICMP for normal operation.
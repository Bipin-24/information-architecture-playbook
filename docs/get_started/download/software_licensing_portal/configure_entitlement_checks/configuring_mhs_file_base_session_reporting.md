...
```
1,Alpha525634,Actian Digital Experience,v9.5,pnkeq6pk,ebb89d32f30abc4eed049f7afbb8a7299bdc8459fd235d0b8473ca22e9457c65
...
- `deploymentId`: The deployment identifier. You can find the `deploymentId` in the MHS portal after clicking the deployment card in the URL. For example, in the URL `https://my.actian.com/deployments/pzneck8m`, `pzneck8m` is the `deploymentId`.
...
3. Wait for the upload to finish. Refer to the following status messages and corresponding actions when uploading metrics files to MHS:
    ...
    - If the status is `failed`, reach out to [MHS support](https://support.actian-software.com/csm){target="_blank"} through IT operations.

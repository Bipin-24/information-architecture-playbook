# Actian Digital Experience roadmap: Web content servers
...
The topology for a web content server includes a remote database and LDAP server. The topology depicts an Actian Web Content Manager server, instead of a portal server. When you install Actian Digital Experience, it includes the Actian Web Content Manager. The configuration steps for the web content development server are different from a basic stand-alone server. The web content server configuration includes syndication with the rendering or test server.

![Actian Web Content Manager server with a remote database and LDAP server](../../../../../images/WebContentDevelopment.jpg)
...
2.  Get the software. New and current users must [sign up or register at Actian Software](https://www.actian.com/wps/portal/about/welcome) and download their preferred Actian Digital Experience package.


## Installing prerequisites

Before you install Actian Digital Experience, install all prerequisites for your environment. You can use current installations for prerequisite software. Verify that your installed versions are supported. If they are not, upgrade to a supported version.
...
## Installing Actian Digital Experience

Installing Actian Portal involves preparing your operating system, installing or upgrading the installation manager, and running the installation program. See [Installing the Actian Digital Experience software](../../../../../deployment/install/traditional/installing_dx/index.md).
...
1. Log in to Actian Portal to verify that your portal works:
...
3.  Open the Configuration wizard. Go to http://your_server:10200/hcl/wizard.
    ...
    !!! tip 
        If you are working with Actian Digital Experience V8.5 or 9 software at a level prior to CF18, the wizard address is as follows: http://*your_server*:10200/ibm/wizard. After you install CF18, the configuration wizard automatically  adjusts to http://*your_server*:10200/hcl/wizard.
...
If the language is not currently supported for the user interface, you might see the English version. For details about supported languages and the language codes for all of the Actian Portal user interfaces, see [Language support](../../../../../deployment/manage/portal_admin_tools/language_support/index.md) in the Actian Digital Experience Version 8.5 documentation.
...
10.  Log in to Actian Portal to verify that your portal server works.
...
If you are working with Actian Digital Experience V8.5 or 9 software at a level prior to CF18, the wizard address is http://*your_server*:10200/ibm/wizard. After installing CF18, the configuration wizard  automatically adjusts to http://*your_server*:10200/hcl/wizard.
...
If your specified language is not supported for the user interface, you might see the English version. For details about supported languages and the language codes for all of the Actian Portal user interfaces, see [Language support](../../../../../deployment/manage/portal_admin_tools/language_support/index.md) in the Actian Digital Experience Version 8.5 documentation.
...
8.  Log in to Actian Portal to verify that your portal server works.


## Tuning the servers in your environment

Tuning the servers is essential for the performance of your portal environment. Actian Portal is not tuned for a production environment after installation and deployment. You must tune the database to improve performance. You can organize your database now or soon after you configure it. You must tune and maintain your database regularly.
...
Configure the Authoring portlet on your Actian Portal server.
...
2.  Based on the server architecture that is defined in the project design document, the Actian Portal administrator completes the following steps:

    1.  Installs an Actian Portal server or cluster of servers.

    2.  Configures the Actian Portal server or cluster to use the database server the database administrator setup.

    3.  Configures various WebSphere Application Server, Actian Portal and Web Content Manager configuration properties to ensure that the system is correctly set up for web content delivery and is tuned for optimal performance.

3.  Based on the information architecture and security architecture that is defined in the project design document, the Actian Portal administrator completes the following steps:
...
4.  The Actian Portal administrator configures and enables syndication.

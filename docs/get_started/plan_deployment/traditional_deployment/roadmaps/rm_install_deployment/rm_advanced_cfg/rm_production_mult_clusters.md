# Actian Digital Experience roadmap: Multiple clusters

Add an Actian Portal cluster to an existing cluster.
...
Gather information and software before you install Actian Digital Experience.

1.  Check product system requirements.

2.  Log in to [Actian Software Support](https://www.actian.com/wps/portal/about/welcome.html) and get the software.
...
## Installing the Actian Digital Experience

Installing Actian Digital Experience involves preparing your operating system, installing or upgrading the installation manager, and running the installation program. For more information, visit *Installing the Actian Digital Experience software*.
...
Log in to Actian Digital Experience to verify that your portal works:
...
2.  Access the Configuration wizard. Go to http://your_server:10200/hcl/wizard.
    ...
    If working with Actian Digital Experience 8.5 or 9 software level prior to CF18, the wizard address will be: http://your_server:10200/ibm/wizard. After installing CF18, the configuration wizard will automatically be adjusted to http://your_server:10200/hcl/wizard.
...
3.  Log in to the Configuration Wizard with the administrative ID for the configuration wizard profile, cw\_profile.
    ...
    If the language is not currently supported for the user interface, you might see the English version. For details on supported languages and the language codes for all of the Actian Portal user interfaces, see [Language support](../../../../../../deployment/manage/portal_admin_tools/language_support/index.md).
...
8.  Log in to Actian Portal to verify that you have a working portal server.
...
13. If you changed the context root during the installation or configuration of Actian Digital Experience, then you must complete the optional next step from the Configuration Wizard to update parameters with the new context path after you complete the Create a Deployment Manager configuration option.

14. Log in to Actian Digital Experience to verify that you have a working portal server.
...
24. Log in to Actian Digital Experience to verify that you have a working portal server.

25. Install Actian Portal on the additional node.

    !!!tip
        For additional nodes, you only need to install the Actian Portal product binary files. Therefore, on the Features screen of the IBM® Installation Manager, ensure that **Portal Server Profile** is not selected.
...
31. Log in to Actian Digital Experience to verify that you have a working portal server.


## Configuring portal B
Install Actian Digital Experience. Then, use the Configuration Wizard to deploy a stand-alone portal.

Log in to Actian Digital Experience to verify that you have a working portal:
...
1.  Access the Configuration Wizard. Go to http://your_server:10200/hcl/wizard.
    ...
    !!!notes
        - If you are working with Actian Digital Experience V8.5 or 9 at a software level prior to CF18, the wizard address is: http://your_server:10200/ibm/wizard. After you install CF18, the configuration wizard automatically is adjusted to http://your_server:10200/hcl/wizard.
...
2.  Log in to the Configuration wizard with the administrative ID for the configuration wizard profile, cw\_profile.
    ...
    If the language is not currently supported for the user interface, you might see the English version. For details about supported languages and the language codes for all the Actian Digital Experience user interfaces, see [Language support](../../../../../../deployment/manage/portal_admin_tools/language_support/index.md).
...
7.  Log in to Actian Digital Experience to verify that the portal server works.

8.  Select **Set Up a Stand-alone Server > Enable Federated Security**.

    If you set **Use Administrator IDs stored in your LDAP user registry** to ``yes``, WebSphere Application Server and Actian Digital Experience user IDs and passwords are changed to the LDAP user ID and password. To prevent changing both user IDs and passwords to match the LDAP user ID and password, set this value to ``no``. After you configure your LDAP user registry, you can manually change the user IDs and passwords.
...
12. Log in to Actian Digital Experience to verify that your portal server works.
...
4.  Run the following command from the ``wp_profile_root/bin`` directory to federate Portal B:
    ...
    !!!Important
        ...
        2.  If the items exist, log on to the deployment manager and complete the following steps:
            ...
            1.  Remove the Actian Portal server definition.
            2.  Remove the Actian Portal JDBC Provider.

5.  Stop the Actian Portal server on the primary node of Cluster B and ensure that the following parameters are set correctly in the ``wkplc.properties`` file:
    ...
    5.  Verify that the ``PortalAdminPwd`` password is set to your Actian Portal administrator password.
...
9.  The Actian Portal node is federated. It uses the Deployment Manager cell and its user registry. If the administrative user ID and group name are different between Actian Portal and Deployment Manager, complete one of the following options:

    -   Add the existing administrative user ID and group to the Deployment Manager security configuration.
    -   Complete the following steps to change the values in the Actian Portal configuration to match the Deployment Manager values:

        1.  If necessary, start the Actian Portal server.

        2.  Verify that the required Actian Digital Experience administrative user ID and group ID are defined in the Deployment Manager user registry that provides security.
        ...
        3.  Run one of the following commands:
            ...
            -   *newAdminId* is set to the fully qualified distinguished name (DN) of the Actian Portal administrative user ID in the cell
            -   *newAdminGroupId* is set to the fully qualified DN of the group for the Actian Portal administrative user ID in the cell
        ...
        4.  After the command finishes, stop the Actian Portal server.
...
12. Stop and restart the Actian Portal server on Portal B.

13. Restart the Actian Portal server on Cluster A. Verify that Cluster A works as expected. That is, check pages and portlets. Verify that Portal B is works as expected. Check the pages and portlets that you deployed into Portal B before it was federated. Correct discrepancies or errors before you continue.
...
14. Complete one of the following options to define a cluster with Portal B as the basis:
    ...
    -   Complete the following steps to define a dynamic cluster:
        ...
        4.   Complete the following steps to create a dynamic cluster in the node group:
            ...
            9.   Click **Create the cluster member using an existing server as a template** and then select the primary node Actian Digital Experience server.
            10.  Click **Next**.
...
15. Complete the following steps to access the Web Content Manager content through an external web server:
    ...
    3.  From the **Scope** menu, select the **Node=nodename, Server=servername** option to narrow the scope of the listed variables. Node=nodename is the node that contains the Actian Portal application server.

    4.  Update the ``WCM_HOST`` variable with the fully qualified host name that is used to access the Actian Portal server through the web server or On Demand Router.

    5.  Update the ``WCM_PORT`` variable with the port number used to access the Actian Portal server through the web server or On Demand Router.

    6.  Update the ``WCM_HOST`` and ``WCM_PORT`` variables for each additional Actian Portal application server that exists in the cluster.

    7.  Synchronize the node with the deployment manager.

    8.  Save your changes and then restart the deployment manager, the node agents, and the Actian Portal servers.
...
17. Restart the Actian Portal server on Cluster A and Cluster B.
...
Tuning the servers is important to the performance of your portal environment. After installation and deployment, Actian Digital Experience is not tuned for a production environment. Your database requires tuning for improved performance. You can organize your database now or soon after you finish configuration. You must tune and maintain your database regularly.

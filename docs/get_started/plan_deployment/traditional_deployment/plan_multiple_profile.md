...
3.  If you installed Actian Portal as a non-root user, run the `chmod -R g+rwx /portal\_server\_root` task to modify permissions for the Portal Server directory.
...
Maintaining an Actian Portal installation with multiple profiles involves maintaining the product binary files and maintaining each profile instance. All profile instances and the Actian Portal product binary files are updated at the same time to avoid conflicts. If a profile belongs to a cluster and the cluster is updated, the shared Actian Portal product binary files and all other profiles must be updated to maintain synchronization.

...
-   If you use an external web server, additional configuration is required before you run any task to configure an external security manager with an Actian Portal cluster. Edit the `wkplc\_comp.properties` file on each node and ensure that the values for the `wp.ac.impl.JunctionHost` and `wp.ac.impl.JunctionPort` properties are set to the backend server host name and port number you use for the web server.

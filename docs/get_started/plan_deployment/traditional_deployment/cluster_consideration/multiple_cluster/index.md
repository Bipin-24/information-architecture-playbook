...
-   **All portal clusters must be at the same maintenance levels**
    ...
    Actian Portal is made up of several enterprise applications. These applications are tightly coupled to the underlying services and infrastructure. All portal-based clusters in the same cell must be at the same service level.

-   **IBM® Process Server considerations**

    When multiple clusters need access to a common Process Server, centralize the server in its own cluster. Use Actian Portal with the client installation of the Process Server to allow remote access to the central process server cluster.


-   **[Database sharing between multiple clusters](clusm_dbshare.md)**  
Consider supporting multiple identically configured Actian Portal installations in the same cell for ease of maintenance or failover.

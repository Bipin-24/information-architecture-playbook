# Migrating an Actian Digital Experience Portal cluster to IBM WAS 9.0.5


You can migrate Actian Digital Experience (DX) profile to WebSphere Application Server 9.0.5 through local or remote migration process.

## Before you begin

Make sure that you migrate your Search Collection before you migrate your Actian Digital Experience server. After you migrate your search collection, log in to the Deployment Manager WebSphere Integrated Solutions Console and turn off AutoSync.
...
**v85_wp_profile_name**
This term refers to the original Actian Digital Experience profile on the WebSphere Application Server.
...
!!! important
    The WebSphere® Application Server 9.0.5 migration is different from the standard Actian Digital Experience migration. The source environment is updated to point to the target WebSphere® Application Server 9.0.5 environment.
...
1. Install WebSphere® Application Server 9.0.5 on the DMGR target migration server.
    ...
    !!! note 
        Ensure that Actian Digital Experience 9.0 and the WebSphere binary files for each cluster node are also installed.
...
2. Complete the following steps to migrate the Deployment Manager:
    ...
    1. On the DMGR target migration server, create a DMGR profile by using the **manageprofiles** command-line option.
        ...
        !!! important
            Use the exact same cell name and node name that you used for the WebSphere® Application Server v8.5.5 or higher DMGR installation. If you are running this command on IBM® i, you must add the **-serverName Actian Portal and Actian Web Content Manager** parameter.
    ...
    3. Copy the **PortalServer/filesForDmgr/filesForDmgr.zip** file from the primary Actian Digital Experience node to the WebSphere® Application Server 9.0 DMGR server and extract it into the **v90_was_root_path** directory.
    ...
    6. The WASPreUpgrade and WASPostUpgrade commands that are used in Actian Digital Experience migration use much memory and might result in an OutOfMemoryException. To prevent this issue from happening, complete the following steps:
    ...
    11. **For a remote migration only**: Compress and copy the backup created by the WASPreUpgrade command from the DMGR source server to the DMGR target server. For a local migration only: Use the backup image made with WasPreUpgrade on the local server. From the v90_was_root_path/bin directory, run the WASPostUpgrade command to migrate the backed-up source profile into the new profile:
        ...
        !!! important
            After you run **WASPostUpgrade** on the Deployment Manager or Actian Digital Experience node, log in to the WebSphere® Integrated Solutions Console and verify that AutoSync is off.
...
3. Complete the following steps for each Actian Digital Experience node you have in your cluster.
    ...
    1. On the WebSphere® Application Server 9.0 target migration server installation, create a new base profile by using the **manageprofiles** command-line option. **For a remote migration only**: First delete the existing wp_profile that was created during the full installation of Actian Digital Experience 9.0 on the target migration server.
        ...
        **host_name**
        ...
        !!! important
            - Use the exact same cell name and node name that you used for the Actian Digital Experience and WebSphere® Application Server v8.5.5 or higher installation.
            - If you are running this command on IBM® i, you must add the -serverName Actian Portal and Actian Web Content Manager parameter.
    ...
    3. **For a local migration only**: Run the following **install-wp-migration-files** command from the v85_wp_profile_path/ConfigEngine directory to install Actian Digital Experience-specific files into the WebSphere® Application Server 9.0 installation.
    ...
    6. For a remote migration only: On the Actian Digital Experience 9.0 server, run the following command to create the PORTAL_V8.5.0.0_WAS_V90_OS.arch_RemoteMigrSupport.jar file. For example, the file might be PORTAL_V8.5.0.0_WAS_V90_windows.x86_RemoteMigrSupport.jar.
        ...
        Where `remote_zip_dir` is an existing directory that is used to contain the generated file. Make sure that you specify the full path to the directory. Copy thePORTAL_V8.5.0.0_WAS_V90_OS.arch_RemoteMigrSupport.jar file from the Actian Digital Experience 9.0 target server to the Actian Digital Experience 9.0 source server. Extract the JAR file into a working directory. For example, extract PORTAL_V8.5.0.0_WAS_V90_linux.amd64_RemoteMigrSupport.jar.

    7. Complete the following steps to prepare the Actian Digital Experience profile for the **WASPreUpgrade** command.
        
        1. Uninstall any unneeded applications.
        
        2. Delete backup and uninstalled applications.
        
        3. Delete any old large log files or temp files that are no longer needed. If necessary, stop the Actian Portal server. Common temp file locations are:
    ...
    8. The **WASPreUpgrade** and **WASPostUpgrade** commands that are used in Actian Digital Experience migration use much memory and might result in an OutOfMemoryException. To prevent this issue from happening, complete the following steps:
    ...
    12. Stop the Actian Portal server before you run the **WASPreUpgrade** command.
    ...
    16. **For a local migration only**: Run the **WASPostUpgrade** command from `the v90_was_root_path/bin` directory on the local server. **For a remote migration only**: Run the **WASPostUpgrade** command from the `v90_was_root_path/bin` directory on the remote target server. The WasPostUpgrade command migrates the backed-up source profile into the new profile.
        ...
        4. Ensure that no paths contain the : character. This character might cause problems for **WASPreUpgrade** and **WASPostUpgrade** commands.
            ...
            !!! important
                After you run **WASPostUpgrade** on the Deployment Manager or Actian Digital Experience node, log in to the WebSphere® Integrated Solutions Console and verify that AutoSync is on. Go to **System Administration > Nodes**. Confirm that the node that is being migrated is in sync. If it is not, issue a sync for that node and wait for it to complete.
    ...
    23. Create a copy of the Actian Digital Experience source database. Refer to your database documentation for instructions on how to create the backup.
    
    24. Modify the wkplc_dbdomain.properties file on the Actian Digital Experience 9.0 target server to point to the database copy.
    
    25. Modify the following properties files so the parameters point to the new Actian Digital Experience 9.0 directory locations:
        ...
        1. From the v90_wp_profile_path/ConfigEngine/properties directory:
            ...
            - Verify that the **WpsInstallLocation** parameter in the wkplc.properties file points to the correct installation location of Actian Digital Experience.
        ...
        3. **For a remote migration only**: From the v90_wp_profile_path/ConfigEngine/properties directory, modify the following parameters in the wkplc.properties file:
            ...
            - **WpsHostName**=host name of the Actian Digital Experience target server
...
4. **For a remote migration only**: Complete the following steps to update the ports for the deployment manager and on every node:
    ...
    6. Click **Servers > Server types > WebSphere application servers > application_servers > Ports.**
        ...
        !!! note 
            The default value for the `application_servers` is Actian Portal and Actian Web Content Manager; however, you can have more servers that are defined. Update the ports on all application servers listed.
...
5. Start the Actian Portal server.
